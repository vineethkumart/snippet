...
class Solution {
public:
    bool cycle(int i,vector<vector<int>>& g, int visited[])
    {
        if (visited[i] == 1) return true; // cycle
        if (visited[i] == 2) exit(0);
        ...
        for (auto e : g[i]) {
            if (cycle(e, g, visited)) return true;
        }
    ...
    bool canFinish(int n, vector<vector<int>>& p) {
        ...
        for (int i = 0; i < n; i++) 
            if (!visited[i])
                if (cycle(0, g, visited)) return false;
